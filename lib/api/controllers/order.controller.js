...
function load(req, res, next, id) {
  Order.findById(id).populate('entries.product').populate('user')
    .then((order) => {
      req.order = order;
      return next();
    })
...
function getAll(req, res) {
  ...
  if ( admin ) {
    Order.find({ }).populate('entries.product').populate('user')
      .then( (results) => res.status( httpStatus.OK ).json( results ) )
      .catch( (error) => res.status( httpStatus.BAD_GATEWAY ).json( error ) );
  }
  else {
    Order.find({ user: userId }).populate('entries.product').populate('user')
      .then( (results) => res.status( httpStatus.OK ).json( results ) )
      .catch( (error) => res.status( httpStatus.BAD_GATEWAY ).json( error ) );
  }
}
